...
def db_payment_expire_old():return db_call("payment/expire_old",{}).get("deleted",0)
...
def invoice_cleanup_worker():
    ...
    while True:
        try:
            n=db_payment_expire_old()
            if n:print(f"[Cleanup Worker] Expired {n} unpaid invoices",flush=True)
...
def send_or_edit(chat_id,text,reply_markup=None,force_new=False):
    """Single-message navigation: edit last message if exists, else send new."""
    mid=last_message_id.get(chat_id)
    if mid and not force_new:
        try:
            edit_message(chat_id,mid,text,reply_markup);return mid
        except Exception as e:
            print(f"[UI] edit failed, sending new: {e}",flush=True)
