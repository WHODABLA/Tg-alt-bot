...
Deposits via NOWPayments · Purchases via TG-Lion · D1 persistent storage
"""
...
def nowpayments_create_payment(user_id: int, amount_usd: float, network: str) -> dict:
    """Create a payment on NOWPayments. Returns the full payment object with pay_address."""
    ...
    return {
        ...
        "price_amount": float(resp.get("price_amount", amount_usd)),
        "payment_status": resp.get("payment_status", "waiting"),
    }


def nowpayments_get_status(payment_id: str) -> dict:
    """Query the status of a payment from NOWPayments."""
...
def payment_watcher_worker() -> None:
    """Fallback poller — checks pending payments every 30s in case webhook didn't fire."""
    print("[Payment Worker] Started. Polling every 30s.", flush=True)
    while True:
        try:
            # We don't have a D1 endpoint to list pending payments,
            # so rely primarily on webhook. This loop only serves as sanity check
            # for payments marked 'waiting' in the last 2 hours by querying NOWPayments directly.
            pass
        except Exception as e:
            print(f"[Payment Worker] Error: {e}", flush=True)
...
def send_deposit_address(chat_id: int, user_id: int, amount: float, network: str) -> None:
    try:
        ...
        text = "\n".join([
            ...
            f"<b>Send exactly:</b> <code>{invoice['pay_amount']} {pay_currency.upper()}</code>",
            "",
            f"<b>Address:</b>",
            ...
            "⚠️ You can send any amount ≥ shown above.",
            "",
            "<i>Your balance will be credited automatically once the network confirms your payment (usually 1–3 minutes).</i>",
        ])
...
def main() -> None:
    ...
    threading.Thread(target=payment_watcher_worker, daemon=True).start()
    print("[Startup] Payment worker started.", flush=True)
