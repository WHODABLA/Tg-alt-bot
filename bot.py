...
Deposits via NOWPayments (fee-buffered) · Telegram notifications · D1 storage · Colored buttons
"""
...
# ═══════════════════════════════════════════════════════════════════
# ─── BUTTON STYLES (Telegram Bot API 9.4+) ───
# ═══════════════════════════════════════════════════════════════════

BTN_PRIMARY = "bg_primary"    # blue — normal navigation
BTN_SUCCESS = "bg_success"    # green — confirm/positive
BTN_DANGER  = "bg_danger"     # red — destructive/cancel
...
NETWORK_TO_CURRENCY = {
    "trc20": ("TRC20 (Tron) — USDT", "usdttrc20"),
    "bep20": ("BEP20 (BNB Chain) — USDT", "usdtbsc"),
    "erc20": ("ERC20 (Ethereum) — USDT", "usdterc20"),
    "eth":   ("ETH (Ethereum) — Native", "eth"),
    "sol":   ("SOL (Solana) — Native", "sol"),
}
...
def db_payment_create(payment_id: str, order_id: str, user_id: int, amount_usd: float,
                      status: str, pay_address: str = "", pay_amount: float = 0.0, pay_currency: str = "") -> None:
    db_call("payment/create", {
        "payment_id": payment_id, "order_id": order_id, "user_id": user_id,
        "amount_usd": amount_usd, "status": status,
        "pay_address": pay_address, "pay_amount": pay_amount,
        "pay_currency": pay_currency, "created_at": int(time.time()),
    })

def db_payment_get(payment_id: str) -> dict | None:
    return db_call("payment/get", {"payment_id": payment_id}).get("payment")

def db_payment_update(payment_id: str, status: str, pay_amount: float | None = None) -> None:
    body = {"payment_id": payment_id, "status": status}
    if pay_amount is not None:
        body["pay_amount"] = pay_amount
    db_call("payment/update", body)

def db_payment_pending_notify() -> list[dict]:
    return db_call("payment/pending_notify", {}).get("payments", [])

def db_payment_mark_notified(payment_id: str) -> None:
    db_call("payment/mark_notified", {"payment_id": payment_id})
...
def request_json(url: str, *, payload: dict | None = None, headers: dict | None = None,
                 timeout: int = 35, context: str = "request") -> Any:
    body = None
    h = {"Accept": "application/json", "User-Agent": "TG-Lion-Bot/1.0"}
    if headers:
        h.update(headers)
...
def btn(text: str, callback_data: str = "", url: str = "",
        emoji_id: str = "", style: str = BTN_PRIMARY) -> dict:
    """Helper to build an inline keyboard button with style."""
...
def nowpayments_create_payment(user_id: int, amount_usd: float, network: str) -> dict:
    if network not in NETWORK_TO_CURRENCY:
        raise RuntimeError(f"Unsupported network: {network}")

    _, pay_currency = NETWORK_TO_CURRENCY[network]
    order_id = f"TL-{uuid.uuid4().hex[:8].upper()}"

    charged_amount = round(amount_usd * (1 + NOWPAYMENTS_FEE_BUFFER), 2)

    payload = {
        "price_amount": charged_amount,
        "price_currency": "usd",
        "pay_currency": pay_currency,
        "order_id": order_id,
        "order_description": f"Deposit for user {user_id}",
        "ipn_callback_url": f"{WORKER_URL}/payment/webhook",
        "payout_address": NOWPAYMENTS_PAYOUT_ADDRESS,
        "payout_currency": NOWPAYMENTS_PAYOUT_CURRENCY,
        "is_fixed_rate": False,
        "is_fee_paid_by_user": False,   # MUST be False for payouts to work
    }
    ...
    resp = request_json(
        ...
        timeout=25,
        context="NOWPayments create_payment",
    )
    ...
    return {
        ...
        "pay_amount": float(resp.get("pay_amount", 0)),
        "pay_currency": resp.get("pay_currency", pay_currency),
        "price_amount": charged_amount,
        "credit_amount": amount_usd,
        "payment_status": resp.get("payment_status", "waiting"),
    }


def nowpayments_get_status(payment_id: str) -> dict:
    url = f"https://api.nowpayments.io/v1/payment/{payment_id}"
    return request_json(
        url,
        headers={"x-api-key": NOWPAYMENTS_API_KEY},
        timeout=15,
        context="NOWPayments get_status",
    )
...
def payment_notifier_worker() -> None:
    print("[Notify Worker] Started.", flush=True)
    while True:
        try:
            payments = db_payment_pending_notify()
            for p in payments:
                ...
                if not pid or not uid:
                    continue
                try:
                    new_bal = get_balance(int(uid))
                    send_message(
                        int(uid),
                        f"✅ <b>Deposit Confirmed!</b>\n\n"
                        f"<b>Amount:</b> {money(amount)}\n"
                        f"<b>New Balance:</b> {money(new_bal)}\n\n"
                        f"<i>Payment ID: {escape(pid)}</i>"
                    )
                    db_payment_mark_notified(pid)
                    print(f"[Notify Worker] Notified user {uid} of ${amount:.2f}", flush=True)
                except Exception as e:
                    print(f"[Notify Worker] Failed for {uid}: {e}", flush=True)
        except Exception as e:
...
def main_menu_keyboard(user_id: int | None = None) -> dict:
    rows = [
        [btn(" Browse Accounts", "menu:countries", emoji_id=PREMIUM_CART_ID, style=BTN_PRIMARY)],
        [btn(" Wallet", "menu:wallet", emoji_id=PREMIUM_WALLET_ID, style=BTN_SUCCESS),
         btn(" My Orders", "menu:orders", emoji_id=PREMIUM_ORDERS_ID, style=BTN_PRIMARY)],
        [btn(" Profile", "menu:profile", emoji_id=PREMIUM_PROFILE_ID, style=BTN_PRIMARY)],
        [btn(" Support", url=SUPPORT_URL, emoji_id=PREMIUM_SUPPORT_ID, style=BTN_PRIMARY)],
    ]
    if user_id and is_admin(user_id):
        rows.append([btn("🛠 Admin Panel", "admin:panel", style=BTN_DANGER)])
    return {"inline_keyboard": rows}


def back_keyboard() -> dict:
    return {"inline_keyboard": [
        [btn("Back to Home", "menu:home", emoji_id=PREMIUM_BACK_ID, style=BTN_PRIMARY)],
    ]}


def back_to_countries_keyboard() -> dict:
    return {"inline_keyboard": [
        [btn("Back to availability", "menu:countries", emoji_id=PREMIUM_BACK_ID, style=BTN_PRIMARY)],
    ]}


def admin_back_keyboard() -> dict:
    return {"inline_keyboard": [
        [btn("🛠 Back to Admin", "admin:panel", style=BTN_PRIMARY)],
        [btn("Back to Home", "menu:home", emoji_id=PREMIUM_BACK_ID, style=BTN_PRIMARY)],
    ]}
    ...
    try:
        ...
        for i in range(0, len(page_c), 2):
            ...
            for c in page_c[i:i + 2]:
                ...
                b = btn(f"{get_flag_emoji(c.code)} {c.code} · {money(shown_price)}",
                        f"country:{c.code}", emoji_id=COUNTRY_PREMIUM_EMOJIS.get(c.code, ""),
                        style=BTN_PRIMARY)
                row.append(b)
            rows.append(row)
        pag = []
        for p in range(1, total_pages + 1):
            pag.append(btn(f"• {p} •" if p == page else str(p), f"page:{p}",
                           emoji_id=PAGE_PREMIUM_EMOJIS.get(p, ""),
                           style=BTN_SUCCESS if p == page else BTN_PRIMARY))
        if pag:
            rows.append(pag)
        rows.append([btn("Back to Home", "menu:home", emoji_id=PREMIUM_BACK_ID, style=BTN_PRIMARY)])
        text = "\n".join([
...
def send_country(chat_id: int, code: str) -> None:
    try:
        ...
        shown_price = apply_margin(c.price_usd)
        rows = [
            [btn("Enter Custom Quantity", f"customqty:{c.code}", style=BTN_SUCCESS)],
            [btn("Back to availability", "menu:countries", emoji_id=PREMIUM_BACK_ID, style=BTN_PRIMARY)],
            [btn("Back to Home", "menu:home", emoji_id=PREMIUM_BACK_ID, style=BTN_PRIMARY)],
        ]
        text = "\n".join([
...
def send_wallet(chat_id: int, user_id: int) -> None:
    ...
    rows = [
        [btn("Deposit Funds", "deposit:prompt", emoji_id=PREMIUM_WALLET_ID, style=BTN_SUCCESS)],
        [btn("Back to Home", "menu:home", emoji_id=PREMIUM_BACK_ID, style=BTN_PRIMARY)],
    ]
    text = "\n".join([
...
def send_deposit_network_menu(chat_id: int, amount: float) -> None:
    rows = [
        [btn("TRC20 (Tron) — USDT", "chain:trc20", emoji_id=PREMIUM_TRC20_ID, style=BTN_PRIMARY)],
        [btn("BEP20 (BNB Chain) — USDT", "chain:bep20", emoji_id=PREMIUM_BEP20_ID, style=BTN_PRIMARY)],
        [btn("ERC20 (Ethereum) — USDT", "chain:erc20", emoji_id=PREMIUM_ERC20_ID, style=BTN_PRIMARY)],
        [btn("ETH (Ethereum) — Native", "chain:eth", emoji_id=PREMIUM_ETH_ID, style=BTN_PRIMARY)],
        [btn("SOL (Solana) — Native", "chain:sol", emoji_id=PREMIUM_SOL_ID, style=BTN_PRIMARY)],
        [btn("Back to Home", "menu:home", emoji_id=PREMIUM_BACK_ID, style=BTN_PRIMARY)],
    ]
    text = "\n".join([
        f"{pemoji(PREMIUM_DEPOSIT_ID, '💳')} <b>Deposit Funds</b>", "",
...
def send_deposit_address(chat_id: int, user_id: int, amount: float, network: str) -> None:
    try:
        ...
        db_payment_create(
            ...
            user_id=user_id,
            amount_usd=invoice["credit_amount"],
        ...
        text = "\n".join([
            ...
            f"<b>You deposit:</b> ${invoice['credit_amount']:.2f} (to your balance)",
            f"<b>Total to send:</b> ${invoice['price_amount']:.2f}",
            "",
            f"<b>Send exactly:</b>",
            f"<code>{invoice['pay_amount']} {pay_currency.upper()}</code>",
            "",
            f"<b>Address:</b>",
            f"<code>{invoice['pay_address']}</code>",
            "",
            f"<b>Payment ID:</b> <code>{invoice['payment_id']}</code>",
            "",
            f"<i>Includes ~{fee_pct:.0f}% processing fee (covers network + conversion costs).</i>",
            "",
            "⚠️ Send on the correct network only.",
            "⚠️ Invoice valid for 60 minutes.",
            "",
            "<i>Your balance will be credited automatically within 1–3 minutes.</i>",
        ])

        rows = [
            [btn("Copy Address", f"copydep:{invoice['pay_address']}", style=BTN_PRIMARY)],
            [btn("Check Status", f"checkpay:{invoice['payment_id']}", style=BTN_PRIMARY)],
            [btn("Back to Home", "menu:home", emoji_id=PREMIUM_BACK_ID, style=BTN_PRIMARY)],
        ]
        send_message(chat_id, text, {"inline_keyboard": rows})
    except Exception as e:
...
def confirm_order(chat_id: int, user_id: int, code: str, quantity: int) -> None:
    try:
        ...
        rows = []
        if bal >= shown_total:
            rows.append([btn("✅ Confirm Purchase", f"confirm:{c.code}:{quantity}", style=BTN_SUCCESS)])
        else:
            rows.append([btn("💳 Deposit Funds", "deposit:prompt", emoji_id=PREMIUM_WALLET_ID, style=BTN_SUCCESS)])
        rows.append([btn("Change Quantity", f"customqty:{c.code}", emoji_id=PREMIUM_BACK_ID, style=BTN_PRIMARY)])
        rows.append([btn("Back to Home", "menu:home", emoji_id=PREMIUM_BACK_ID, style=BTN_PRIMARY)])
...
def create_order(chat_id: int, user_id: int, code: str, quantity: int = 1) -> None:
    try:
        ...
        rows = [
            [btn("📩 Get Code", f"getcode:{order.order_id}:0", style=BTN_SUCCESS)],
            [btn("View my orders", "menu:orders", style=BTN_PRIMARY)],
            [btn("Back to Home", "menu:home", emoji_id=PREMIUM_BACK_ID, style=BTN_PRIMARY)],
        ]
        text = "\n".join([
...
def send_admin_panel(chat_id: int) -> None:
    ...
    rows = [
        [btn("👥 Users", "admin:users", style=BTN_PRIMARY),
         btn("📦 Orders", "admin:orders", style=BTN_PRIMARY)],
        [btn("💳 Credit User", "admin:credit", style=BTN_SUCCESS),
         btn("💸 Debit User", "admin:debit", style=BTN_DANGER)],
        [btn("🚫 Ban User", "admin:ban", style=BTN_DANGER),
         btn("✅ Unban User", "admin:unban", style=BTN_SUCCESS)],
        [btn("📊 Set Profit Margin", "admin:margin", style=BTN_PRIMARY)],
        [btn("Broadcast", "admin:broadcast", emoji_id=PREMIUM_BROADCAST_ID, style=BTN_PRIMARY)],
        [btn("Back to Home", "menu:home", emoji_id=PREMIUM_BACK_ID, style=BTN_PRIMARY)],
    ]
    send_message(chat_id, text, {"inline_keyboard": rows})
...
def do_broadcast(chat_id: int, text: str) -> None:
    """Send exactly what the admin typed — no header added."""
    ...
    for u in users:
        uid = u["user_id"]
        try:
            send_message(uid, formatted)   # <-- send only the typed text
            sent += 1
            time.sleep(0.05)
        except Exception:
            failed += 1
...
def handle_update(update: dict) -> None:
    ...
    if cb:
        ...
        if action == "menu" and value == "home":
            ...
            try:
                status = nowpayments_get_status(value)
                send_message(chat_id,
                    f"📊 <b>Payment Status</b>\n\n"
                    f"<b>ID:</b> <code>{escape(value)}</code>\n"
                    f"<b>Status:</b> {status.get('payment_status', 'unknown')}")
            except Exception as e:
                send_message(chat_id, error_text(e), back_keyboard())
            ...
            try:
                ...
                try:
                    ...
                    retry = {"inline_keyboard": [
                        [btn("🔄 Retry", f"getcode:{value}:{idx}", style=BTN_PRIMARY)],
                        [btn("View my orders", "menu:orders", style=BTN_PRIMARY)],
                    ]}
                ...
                if has_more:
                    btns = [
                        [btn(f"📩 Get Code for #{next_idx + 1}",
                             f"getcode:{value}:{next_idx}", style=BTN_SUCCESS)],
                        [btn("View my orders", "menu:orders", style=BTN_PRIMARY)],
                        [btn("Back to Home", "menu:home", emoji_id=PREMIUM_BACK_ID, style=BTN_PRIMARY)],
                    ]
                else:
...
def main() -> None:
    ...
    for name, val in [
        ...
        ("NOWPAYMENTS_PAYOUT_ADDRESS", NOWPAYMENTS_PAYOUT_ADDRESS),
    ]:
        if not val:
            missing.append(name)
    if missing:
    ...
    print(f"Fee buffer: {NOWPAYMENTS_FEE_BUFFER * 100:.0f}%", flush=True)
    ...
    threading.Thread(target=payment_notifier_worker, daemon=True).start()
    print("[Startup] Payment notifier started.", flush=True)
