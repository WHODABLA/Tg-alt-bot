...
Deposits via NOWPayments (fee-buffered) · Telegram notifications · D1 storage
"""
...
NOWPAYMENTS_FEE_BUFFER = float(os.getenv("NOWPAYMENTS_FEE_BUFFER", "0.06"))  # 6%
...
# ═══════════════════════════════════════════════════════════════════
# ─── NOWPAYMENTS API (with fee buffer) ───
# ═══════════════════════════════════════════════════════════════════

def nowpayments_create_payment(user_id: int, amount_usd: float, network: str) -> dict:
    """Create a payment. User is credited `amount_usd` but pays `amount_usd * (1 + buffer)`."""
    ...
    # Inflate the price to cover fees
...
# ═══════════════════════════════════════════════════════════════════
# ─── PAYMENT NOTIFIER WORKER ───
# ═══════════════════════════════════════════════════════════════════

def payment_notifier_worker() -> None:
    """Polls D1 for payments marked 'credited' that haven't been notified yet."""
...
def send_deposit_network_menu(chat_id: int, amount: float) -> None:
    rows = [
        ...
        [{"text": "SOL (Solana) — Native", "callback_data": "chain:sol",
...
def send_deposit_address(chat_id: int, user_id: int, amount: float, network: str) -> None:
    try:
        ...
        rows = [
            [{"text": "Copy Address", "callback_data": f"copydep:{invoice['pay_address']}"}],
            [{"text": "Check Status", "callback_data": f"checkpay:{invoice['payment_id']}"}],
            [{"text": "Back to Home", "callback_data": "menu:home",
              "icon_custom_emoji_id": PREMIUM_BACK_ID}],
        ]
        send_message(chat_id, text, {"inline_keyboard": rows})
