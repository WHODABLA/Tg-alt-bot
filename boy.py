#!/usr/bin/env python3
"""
TG-Lion Market Bot — Full System
Auto-Deposit · Purchase · Profit Margin · Admin Panel · Premium UI
"""
...
TELEGRAM_TOKEN = "8822527156:AAFQVLR8z5AIYuQSjFu8fzyFfkl2JkJmTcg"
TG_LION_API_KEY = "lbnjf98a3MH65z7S4k"
TG_LION_USER_ID = "6742599309"

TRC20_ADDRESS = "TNQU6Xyu1EZB1oBJ7JQXMcqfH6wjCfoikP"
TRC20_CONTRACT = "TR7NHqjeKQxGTCi8q8ZY4pL8otSzgjLj6t"

EVM_ADDRESS = "0x45bbDEa2E983FE46360E2582DFCd25e443595969"
ERC20_CONTRACT = "0xdAC17F958D2ee523a2206206994597C13D831ec7"
BEP20_CONTRACT = "0x55d398326f99059fF775485246999027B3197955"

SOL_ADDRESS = "4dWSgwjkDJnT3xw7Sorky2jqnc3hZbFJSiV1VeQLAu51"
SOL_USDC_MINT = "EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v"

TRONGRID_API_KEY = "31442aee-ad48-4ece-8401-53aacb8dfc74"
ETHERSCAN_API_KEY = "J1F4CC5QN5S46X3EAEVBGJRFQI8JVQ1J9D"
SOL_RPC_URL = "https://mainnet.helius-rpc.com/?api-key=2202de44-0f1f-47ed-b038-6a26f64ee791"
BSC_RPC_URL = "https://rpc.ankr.com/bsc/370882247f5be885e9b79d1171e65c187bc8af2212333a14141ed3a8fb5d1558"

ADMIN_IDS = [6742599309]
MIN_DEPOSIT = 2.00
POLL_INTERVAL = 60
INVOICE_EXPIRY_SECONDS = 3600
SUPPORT_URL = "https://t.me/YourSupportHandle"
...
# Pagination (used for page numbers)
PAGE_PREMIUM_EMOJIS: dict[int, str] = {
    1: "5235776368905562305",
    2: "5237704680372447424",
    3: "5238044171767393675",
    4: "5235533321001250232",
    5: "5238171599152097811",
    6: "5235500881113263583",
    7: "5237875542761417785",
    8: "5238067300166281132",
    9: "5237872922831367023",
    10: "",
}
...
def pemoji(emoji_id: str | None, fallback: str) -> str:
    """Render a premium emoji, or fall back to a plain Unicode char."""
...
def _cleanup_expired_invoices() -> None:
    now = time.time()
    expired = [a for a, (_, c) in pending_deposits.items() if now - c > INVOICE_EXPIRY_SECONDS]
    for a in expired:
        del pending_deposits[a]
    if expired:
        print(f"[Deposit] Expired {len(expired)} stale invoices.", flush=True)
...
def create_deposit_invoice(user_id: int, requested_amount: float) -> float:
    ...
    for offset in range(100):
        ...
        if candidate not in pending_deposits:
            pending_deposits[candidate] = (user_id, now)
            return candidate
    raise RuntimeError("All invoice amounts are in use. Try again later.")


def find_user_by_deposit_amount(amount: float) -> int | None:
    for expected, (uid, _) in list(pending_deposits.items()):
        if abs(expected - amount) < 0.005:
            del pending_deposits[expected]
            return uid
    return None
...
def check_bep20_ankr() -> list[dict]:
    ...
    if last_bsc_block is None:
        last_bsc_block = current_block - 500
                ...
                if user_id:
                    ...
                    try:
                        send_message(user_id,
                            ...
                            f"<b>New Balance:</b> ${wallets[user_id]:.2f}\n\n"
                            f"<i>TX: {tx_id[:12]}...</i>")
                    except Exception:
                        pass
...
def send_home(chat_id: int, first_name: str | None = None, user_id: int | None = None) -> None:
    text = "\n".join([
        f"{pemoji(PREMIUM_BALANCE_ID, '💰')} <b>Your Balance:</b> ₹0.00",
...
def send_orders(chat_id: int, user_id: int) -> None:
    uo = sorted((o for o in orders.values() if o.telegram_user_id == user_id),
    ...
    lines = [
        ...
        f"{escape(o.country_name)} · {o.quantity} pc · {money(o.amount_usd)}\n"
        f"Status: {o.status}"
        for o in uo[:8]
    ]
    send_message(
...
def send_profile(chat_id: int, user_id: int, first_name: str | None = None) -> None:
    banned_status = "🚫 Banned" if is_banned(user_id) else "✅ Active"
    bal = wallets.get(user_id, 0.0)
    text = "\n".join([
...
def create_order(chat_id: int, user_id: int, code: str, quantity: int = 1) -> None:
    global total_spent, total_profit
    try:
        c = find_country(code)
        if not c:
            raise RuntimeError("That option is no longer available.")
        ...
        wallets[user_id] = bal - shown_total
        ...
        try:
            for _ in range(quantity):
                nums.append(buy_number(c.code))
        except Exception as be:
            refund = shown_total
            wallets[user_id] = wallets.get(user_id, 0.0) + refund
            order.status = "refunded"
            raise RuntimeError(f"Purchase failed. Refunded {money(refund)}.\nReason: {be}")

        order.status = "awaiting_code"
        details = [f"<b>{i}.</b> <code>{escape(n.get('Number', 'N/A'))}</code>" for i, n in enumerate(nums, 1)]
        purchased_numbers_store[order.order_id] = nums
        rows = [
        ...
        text = "\n".join([
            ...
            f"<b>Your Wallet Balance:</b> {money(wallets[user_id])}", "",
...
def send_admin_users(chat_id: int) -> None:
    if not known_users:
        ...
        return
    lines = []
    for uid in list(known_users)[:30]:
        name = user_names.get(uid, "Unknown")
        bal = wallets.get(uid, 0.0)
...
def send_admin_orders(chat_id: int) -> None:
    if not orders:
        ...
        return
    recent = sorted(orders.values(), key=lambda o: o.created_at, reverse=True)[:15]
    lines = []
    for o in recent:
        lines.append(
            f"<b>{escape(o.order_id)}</b>\n"
            f"User: <code>{o.telegram_user_id}</code>\n"
...
def prompt_admin_margin(chat_id: int, admin_id: int) -> None:
    ...
    send_message(
        ...
        f"Current: <b>{profit_margin * 100:.0f}%</b>\n\n"
        f"Send the new margin as a percentage.\n"
        f"Example: <code>50</code> for 50%\n"
        f"Example: <code>75</code> for 75%",
        admin_back_keyboard(),
    )
...
def prompt_admin_broadcast(chat_id: int, admin_id: int) -> None:
    ...
    send_message(
        ...
        "Example: <code>Hello &lt;emoji:5368324170671202286&gt; everyone!</code>",
        admin_back_keyboard(),
    )


def do_broadcast(chat_id: int, text: str) -> None:
    """Broadcast message supporting <emoji:ID> premium emoji tags."""
...
def handle_update(update: dict) -> None:
    ...
    if cb:
        ...
        if user.get("first_name"):
            user_names[user_id] = user.get("first_name")
        ...
        if action == "admin":
            ...
            if value == "panel":
                ...
                send_admin_users(chat_id)
            elif value == "orders":
                send_admin_orders(chat_id)
            elif value == "credit":
                prompt_admin_credit(chat_id, user_id)
            elif value == "debit":
                prompt_admin_debit(chat_id, user_id)
            elif value == "ban":
                prompt_admin_ban(chat_id, user_id)
            elif value == "unban":
                prompt_admin_unban(chat_id, user_id)
            elif value == "margin":
                prompt_admin_margin(chat_id, user_id)
            elif value == "broadcast":
                prompt_admin_broadcast(chat_id, user_id)
            return
        ...
        if action == "menu" and value == "home":
            ...
            try:
                idx = int(extra)
                order = orders.get(value)
                if not order:
                    raise RuntimeError("Order not found.")
                nums = purchased_numbers_store.get(value, [])
                if idx >= len(nums):
                    raise RuntimeError("All codes delivered.")
                phone = nums[idx].get("Number")
    ...
    if is_admin(user_id) and user_id in pending_admin_input:
        ...
        if act == "credit":
            try:
                ...
                wallets[target] = wallets.get(target, 0.0) + amt
                send_message(chat_id, f"✅ Credited {money(amt)} to <code>{target}</code>.\nNew balance: {money(wallets[target])}", admin_back_keyboard())
                try:
                    send_message(target, f"💳 <b>Balance Credited</b>\n\n+{money(amt)}\nNew balance: {money(wallets[target])}")
                except Exception:
                    pass
            except Exception as e:
        ...
        if act == "debit":
            try:
                p = text.strip().split()
                target, amt = int(p[0]), float(p[1])
                wallets[target] = max(0.0, wallets.get(target, 0.0) - amt)
...
def main() -> None:
    ...
    print("Auto-deposit worker enabled.", flush=True)
    try:
